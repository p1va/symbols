  ...
  createDebugTestSuite(): void {
    describe(`${this.config.name} Debug Tests`, () => {
      ...
      test('Debug: Read file symbols with details', async () => {
        ...
        const result = await this.client.readSymbols(this.getMainFilePath(), 99, 'none', true);
      ...
      test('Debug: Get diagnostics with details', async () => {
        ...
        const result = await this.client.getDiagnostics(this.getMainFilePath(), undefined, undefined, true);
