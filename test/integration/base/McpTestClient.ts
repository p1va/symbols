...
export class McpTestClient {
  ...
  async connect(timeoutMs = 15000): Promise<void> {
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error(`Connection timeout after ${timeoutMs}ms (working dir: ${this.workingDirectory || 'default'})`));
      }, timeoutMs);
  ...
  async callTool(name: string, args: Record<string, unknown>, debug = false): Promise<ToolCallResult> {
    try {
      if (debug) {
        console.log(`[DEBUG] Calling tool '${name}' with args:`, JSON.stringify(args, null, 2));
      }
      ...
      if (debug) {
        console.log(`[DEBUG] Tool '${name}' result:`, {
          ...
          contentLength: Array.isArray(result.content) ? result.content.length : 'not array',
  ...
  async inspect(position: SymbolPosition, debug = false): Promise<ToolCallResult> {
    return this.callTool('inspect', { ...position }, debug);
  }

  async getReferences(position: SymbolPosition, debug = false): Promise<ToolCallResult> {
    return this.callTool('references', { ...position }, debug);
  }

  async getCompletion(position: SymbolPosition, debug = false): Promise<ToolCallResult> {
    return this.callTool('completion', { ...position }, debug);
  }

  async getDiagnostics(file: string, maxDepth?: number, previewMode?: string, debug = false): Promise<ToolCallResult> {
  ...
  async readSymbols(file: string, maxDepth?: number, previewMode?: string, debug = false): Promise<ToolCallResult> {
    const args: Record<string, unknown> = { file };
    if (maxDepth !== undefined) args.maxDepth = maxDepth;
    if (previewMode) args.previewMode = previewMode;
  ...
  async renameSymbol(position: SymbolPosition, newName: string): Promise<ToolCallResult> {
