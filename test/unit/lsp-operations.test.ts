...
import { validateSymbolPositionRequest, validateFileRequest, validateWorkspaceOperation } from '../../src/validation.js';
import { executeWithCursorContext, executeWithExplicitLifecycle } from '../../src/lsp/fileLifecycle/index.js';
...
const mockValidateSymbolPositionRequest = validateSymbolPositionRequest as MockedFunction<typeof validateSymbolPositionRequest>;
const mockValidateFileRequest = validateFileRequest as MockedFunction<typeof validateFileRequest>;
const mockValidateWorkspaceOperation = validateWorkspaceOperation as MockedFunction<typeof validateWorkspaceOperation>;
const mockExecuteWithCursorContext = executeWithCursorContext as MockedFunction<typeof executeWithCursorContext>;
const mockExecuteWithExplicitLifecycle = executeWithExplicitLifecycle as MockedFunction<typeof executeWithExplicitLifecycle>;
const mockTryResult = tryResult as MockedFunction<typeof tryResult>;
const mockTryResultAsync = tryResultAsync as MockedFunction<typeof tryResultAsync>;
...
function createMockContext(overrides: Partial<LspContext> = {}): LspContext {
  return {
    ...
    workspaceState: { isReady: true, isLoading: false, loadingStartedAt: undefined },
...
const VALIDATION_SUCCESS = {
  symbolPosition: { valid: true as const, absolutePath: TEST_CONSTANTS.workspacePath },
  fileRequest: { valid: true as const, absolutePath: TEST_CONSTANTS.workspacePath },
...
const VALIDATION_ERRORS = {
  positionOutOfBounds: {
    ...
    error: { errorCode: ValidationErrorCode.PositionOutOfBounds, message: 'Position out of bounds' },
  },
  workspaceNotReady: {
    valid: false as const,
    error: { errorCode: ValidationErrorCode.WorkspaceNotReady, message: 'Workspace not ready' },
  },
  invalidPath: {
    valid: false as const,
    error: { errorCode: ValidationErrorCode.InvalidPath, message: 'File not found' },
  },
} as const;
...
describe('LSP Operations', () => {
  ...
  describe('inspectSymbol', () => {
    it('should perform all inspection operations successfully', async () => {
      ...
      const request: SymbolPositionRequest = { file: TEST_CONSTANTS.testFile, position: TEST_CONSTANTS.testPosition };

      mockValidateSymbolPositionRequest.mockResolvedValue(VALIDATION_SUCCESS.symbolPosition);
      const mockConnection = ctx.client.connection as MockConnection;
      mockConnection.sendRequest.mockResolvedValue(null);
      
      mockExecuteWithCursorContext.mockImplementation(async (_op, _client, _path, _pos, _files, _strategy, callback) => {
        if (callback) return { ok: true, data: await callback(TEST_CONSTANTS.mockUri, undefined) };
      ...
      expect(mockValidateSymbolPositionRequest).toHaveBeenCalledWith(ctx, request);
      ...
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('textDocument/hover', expectedParams);
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('textDocument/definition', expectedParams);
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('textDocument/typeDefinition', expectedParams);
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('textDocument/implementation', expectedParams);
    });

    it('should handle validation errors', async () => {
      const ctx = createMockContext();
      const request: SymbolPositionRequest = { file: TEST_CONSTANTS.nonExistentFile, position: TEST_CONSTANTS.testMinPosition };

      mockValidateSymbolPositionRequest.mockResolvedValue(VALIDATION_ERRORS.positionOutOfBounds);
    ...
    it('should convert coordinates correctly for different positions', async () => {
      const ctx = createMockContext();
      const request: SymbolPositionRequest = { file: TEST_CONSTANTS.testFile, position: TEST_CONSTANTS.testLargePosition };
    ...
    it('should handle partial LSP failures gracefully', async () => {
      const ctx = createMockContext();
      const request: SymbolPositionRequest = { file: TEST_CONSTANTS.testFile, position: TEST_CONSTANTS.testMinPosition };
  ...
  describe('searchSymbols', () => {
    it('should search workspace symbols successfully', async () => {
      ...
      mockValidateWorkspaceOperation.mockReturnValue(VALIDATION_SUCCESS.workspaceOperation);
      const mockConnection = ctx.client.connection as MockConnection;
      const mockSymbols = [
        { name: 'TestClass', kind: 5, location: { uri: 'file:///test.ts', range: {} } },
        { name: 'TestMethod', kind: 6, location: { uri: 'file:///test.ts', range: {} } },
      ];
      ...
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('workspace/symbol', { query: 'TestClass' });
    });
    ...
    it('should handle workspace validation errors', async () => {
      ...
      mockValidateWorkspaceOperation.mockReturnValue(VALIDATION_ERRORS.workspaceNotReady);
    ...
    it('should handle LSP request failures', async () => {
      ...
      mockValidateWorkspaceOperation.mockReturnValue(VALIDATION_SUCCESS.workspaceOperation);
      const mockConnection = ctx.client.connection as MockConnection;
      mockConnection.sendRequest.mockRejectedValue(new Error('LSP search failed'));
      
      mockTryResultAsync.mockImplementation(async (fn, errorHandler) => {
        try { return await fn(); }
        catch (error) { if (errorHandler) throw errorHandler(error); throw error; }
      });

      await expect(searchSymbols(ctx, request)).rejects.toThrow('Workspace symbol search failed: LSP search failed');
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('workspace/symbol', { query: 'TestClass' });
    });
    ...
    it('should handle various symbol types in results', async () => {
      ...
      const mockSymbols = [
        { name: 'TestClass', kind: 5, location: { uri: 'file:///test.ts', range: {} } },
        { name: 'testFunction', kind: 12, location: { uri: 'file:///utils.ts', range: {} } },
        { name: 'TEST_CONSTANT', kind: 14, location: { uri: 'file:///constants.ts', range: {} } },
      ];
      mockConnection.sendRequest.mockResolvedValue(mockSymbols);
      mockTryResultAsync.mockImplementation(async (fn) => await fn());
      ...
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('workspace/symbol', { query: 'test' });
    });
  });
  ...
  describe('readSymbols', () => {
    it('should read document symbols successfully', async () => {
      ...
      const mockConnection = ctx.client.connection as MockConnection;
      const mockSymbols = [
        { name: 'TestClass', kind: 5, range: { start: { line: 0, character: 0 }, end: { line: 0, character: 15 } }, children: [] },
      ];
      mockConnection.sendRequest.mockResolvedValue(mockSymbols);
      
      mockExecuteWithExplicitLifecycle.mockImplementation(async (_client, _path, _files, _strategy, callback) => {
        if (callback) return { ok: true, data: await callback(TEST_CONSTANTS.mockUri) };
        throw new Error('No callback provided');
      });
      mockTryResultAsync.mockImplementation(async (fn) => await fn());
      ...
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('textDocument/documentSymbol', {
        textDocument: { uri: TEST_CONSTANTS.mockUri },
      });
    });
    ...
    it('should handle LSP request failures', async () => {
      ...
      mockValidateFileRequest.mockReturnValue(VALIDATION_SUCCESS.fileRequest);
      const mockConnection = ctx.client.connection as MockConnection;
      mockConnection.sendRequest.mockRejectedValue(new Error('Document symbol request failed'));
      ...
      await expect(readSymbols(ctx, request)).rejects.toThrow('Document symbol request failed: Document symbol request failed');
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('textDocument/documentSymbol', {
        textDocument: { uri: TEST_CONSTANTS.mockUri },
      });
    });
    ...
    it('should handle empty symbol responses', async () => {
      ...
      mockValidateFileRequest.mockReturnValue({ valid: true, absolutePath: '/test/workspace/src/empty.ts' });
      const mockConnection = ctx.client.connection as MockConnection;
      ...
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('textDocument/documentSymbol', { textDocument: { uri: mockUri } });
    });
  });
  ...
  describe('findReferences', () => {
    it('should find all symbol references successfully', async () => {
      ...
      const mockReferences = [
        { uri: 'file:///test/workspace/src/test.ts', range: { start: { line: 9, character: 14 }, end: { line: 9, character: 24 } } },
        { uri: 'file:///test/workspace/src/other.ts', range: { start: { line: 5, character: 10 }, end: { line: 5, character: 20 } } },
      ];
    ...
    it('should handle LSP request failures', async () => {
      ...
      mockValidateSymbolPositionRequest.mockResolvedValue(VALIDATION_SUCCESS.symbolPosition);
      const mockConnection = ctx.client.connection as MockConnection;
      mockConnection.sendRequest.mockRejectedValue(new Error('References failed'));
      ...
      await expect(findReferences(ctx, request)).rejects.toThrow('Find references failed: References failed');
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('textDocument/references', {
        textDocument: { uri: TEST_CONSTANTS.mockUri },
  ...
  describe('completion', () => {
    it('should get completion suggestions successfully (array format)', async () => {
      ...
      const mockCompletions = [
        { label: 'testMethod', kind: 2, detail: 'string', documentation: 'A test method', insertText: 'testMethod()' },
        { label: 'testProperty', kind: 10, detail: 'number', documentation: 'A test property' },
      ];
      ...
      expect(mockValidateSymbolPositionRequest).toHaveBeenCalledWith(ctx, request);
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('textDocument/completion', {
        textDocument: { uri: TEST_CONSTANTS.mockUri },
        position: { line: 4, character: 9 }, // Converted to 0-based
    ...
    it('should handle CompletionList response format', async () => {
      ...
      const mockCompletionList = {
        isIncomplete: false,
        items: [{ label: 'testFunction', kind: 3, detail: 'void', documentation: 'A test function' }],
      };
    ...
    it('should handle LSP request failures', async () => {
      ...
      mockValidateSymbolPositionRequest.mockResolvedValue(VALIDATION_SUCCESS.symbolPosition);
      const mockConnection = ctx.client.connection as MockConnection;
      mockConnection.sendRequest.mockRejectedValue(new Error('Completion failed'));
      ...
      await expect(completion(ctx, request)).rejects.toThrow('Code completion failed: Completion failed');
      expect(mockConnection.sendRequest).toHaveBeenCalledWith('textDocument/completion', {
        textDocument: { uri: TEST_CONSTANTS.mockUri },
        position: { line: 0, character: 0 }, // Converted to 0-based
        context: { triggerKind: 1 },
      ...
      const mockCompletions = [
        {
          ...
          textEdit: {
            range: { start: { line: 0, character: 0 }, end: { line: 0, character: 5 } }, // 0-based from LSP
  ...
  describe('logs', () => {
    it('should retrieve log messages from windowLogStore successfully', () => {
      ...
      const mockWindowLogStore = ctx.windowLogStore as { getMessages: MockedFunction<() => LogMessage[]> };
      ...
      expect(mockTryResult).toHaveBeenCalledWith(expect.any(Function), expect.any(Function));
    });
    ...
    it('should handle windowLogStore errors gracefully', () => {
      ...
      mockTryResult.mockImplementation((fn, errorHandler) => {
        ...
        catch (error) { if (errorHandler) return { ok: false, error: errorHandler(error) }; throw error; }
      });
